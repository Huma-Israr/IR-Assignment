...
def parse_files(fold=1, mode="training", first_n_files=2946, ml=object, config=object, prep=object):
...
def training(config, test_fold, ml, prep):
    ...
    for fold in folds:
        parse_files(fold=fold, mode='training', first_n_files=2946, ml=ml, config=config, prep=prep)


# Let's test on the remaining folder
def testing(config, test_fold, ml, ev, prep):
    parse_files(fold=test_fold, mode='testing', first_n_files=500, ml=ml, config=config, prep=prep)
    ml.compare_queries()


def testing_new(config, test_new_fold, ml, ev, prep):
    parse_files(fold=test_new_fold, mode='testing', first_n_files=260, ml=ml, config=config, prep=prep)
    ml.compare_queries()
...
def main():
   ...
   # ml.add_doc(doc_id='d1', doc_class='spam', doc_terms=['bye', 'goodbye', 'see-ya'])
   # new_fold='/home/huma/Downloads/irlib-0.1.1/irlib/all-folds/fold/'
    #q_id= get_doc_id(new_fold,"testdoc_1.txt")
    #print q_id
    #fd = open('/home/huma/Downloads/irlib-0.1.1/irlib/all-folds/fold/testdoc_1.txt', 'r')
    #for line in fd.readlines():
    #    terms = prep.ngram_tokenizer(line)
    #terms = prep.ngram_tokenizer(text=q)
    #print terms
    #ml.calculate_proto_classes()
    #ml.add_query(query_id=1, query_class="n/a", query_terms=terms)
    #ml.calculate_training_data()
    #ml.display_idx()
    #ab=
    #ml.calculate_proto_classes()
    ...
    #ml.display_idx()
    #numpy.savetxt('/home/huma/Downloads/irlib-0.1.1/irlib/all-folds/fold/matrix.txt', ml.display_idx(), delimiter=',')
...
#config = classifier.Configuration(config_file='/home/huma/Downloads/irlib-0.1.1/irlib/classify.conf')
#config.load_configuration()
''''
