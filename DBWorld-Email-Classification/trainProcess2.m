...
files_train = dir('C:\Users\Home\Desktop\IR Term Project\IR Project\dbworld emails dataset\train_folder\*.txt');

len = length(files_train);
data = struct([]); 
data{len} = [];
for k = 1:len
data{k} =  textread(fullfile('C:\Users\Home\Desktop\IR Term Project\IR Project\dbworld emails dataset\train_folder\',files_train(k).name), '%s', 'delimiter', ' ');
if (k == 1)
    temp = data{k};
    [uv,~,idx] = unique(temp);
    n = accumarray(idx(:),1);
